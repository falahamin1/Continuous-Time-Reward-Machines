...
python3 Comparison.py --env firefighter-synch --reward_shaping no --specify_dimension yes --rows 7 --columns 7 --runs 10 --threshold 0.8 --max_episodes 10000 --update_frequency 10 --discount_factor 0.01 --episode_len 1000 --learning_rate 0.2 --save_file firefightersync-tabular-server --buffer_size 75000  --save_data firefightersync-tabular-server-data --deep_rl no --epsilon 0.8
